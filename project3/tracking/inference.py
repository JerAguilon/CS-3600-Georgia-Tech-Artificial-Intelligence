...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                d = abs(p[0] - pacmanPosition[0]) + abs(p[1] - pacmanPosition[1])
                allPossible[p] = emissionModel[d] * self.beliefs.get(p)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p_0 in self.legalPositions:
            gs_1 = self.setGhostPosition(gameState, p_0)
            d_1 = self.getPositionDistribution(gs_1)
            # print(d_1)
            for p_1, prb in d_1.items():
                allPossible[p_1] += self.beliefs[p_0] * prb
        self.beliefs = allPossible
